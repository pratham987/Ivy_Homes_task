...
API_URL = "http://35.200.185.69:8000/v1/autocomplete"  # Base URL without query parameter
HEADERS = {"User-Agent": "Mozilla/5.0"}

# API rate limit settings
MAX_REQUESTS_PER_MIN = 100
REQUEST_INTERVAL = 60 / MAX_REQUESTS_PER_MIN  # 0.6 sec/request
...
def next_lexicographic_string(s):
    """Generate the next lexicographical string after `s`."""
    if not s:
        return ""
    # If the last character is 'z', we need to handle differently
    if s[-1] == 'z':
        return s + 'a'
...
async def scrape_autocomplete():
    ...
    priority_queue = []
    heapq.heappush(priority_queue, "a")
